...
function playerMove(playerId: number, direction: number) {
...
window.addEventListener("keydown", (event) => {
    ...
    if (isRunning) {
        if (event.key === "q") playerMove(0, 0);
        if (event.key === "s") playerMove(0, 1);
        if (event.key === "p") playerMove(0, 1);
        if (event.key === "l") playerMove(1, 1);
    } else {
